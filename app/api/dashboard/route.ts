...
export async function GET(req: NextRequest) {
  try {
    ...
    const campusId = parseInt(searchParams.get('campus_id') || '1'); // Default to 1
    ...
    const POPULATION_ESTIMATE = 1000; // Hardcoded per user request

    // Fetch data for current and previous period, joined with Locations to filter by campus
    ...
    const currDiversionRate = currTotalManaged > 0 ? (currRecyclables / currTotalManaged) * 100 : 0;
    
    // Per Capita = (TotalWaste * 1000) / (Populasi * Hari) -> grams/person/day
    const currPerCapita = (currTotalManaged * 1000) / (POPULATION_ESTIMATE * daysInMonth);
    ...
    const prevDiversionRate = prevTotalManaged > 0 ? (prevRecyclables / prevTotalManaged) * 100 : 0;
    const prevPerCapita = (prevTotalManaged * 1000) / (POPULATION_ESTIMATE * getDaysInMonth(prevPeriod));
    ...
    const momResidualVol = calcChange(currTotalResidualVol, prevTotalResidualVol);
    ...
    const topLocationsArray = Object.values(locationMap).map(loc => {
      ...
      return {
        name: loc.name,
        total: loc.total,
        percentage: currTotalManaged > 0 ? (loc.total / currTotalManaged) * 100 : 0,
