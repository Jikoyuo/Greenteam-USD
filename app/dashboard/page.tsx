...
export default function DashboardPage() {
  ...
  const SummaryCard = ({ title, value, unit, change, icon }: any) => {
    ...
    return (
      <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col justify-between">
        ...
        <div>
          <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-semibold bg-emerald-100 text-emerald-700`}>
             {isPositive ? <ArrowUp className="w-3 h-3" /> : (isNeutral ? "" : <ArrowDown className="w-3 h-3" />)}
  ...
  const compositionData = data ? [
    { name: "Plastik", value: data.composition.plastic, color: "#006699", textClass: "text-[#006699]" },
    { name: "Kertas & Kardus", value: data.composition.paper, color: "#66b3ff", textClass: "text-[#66b3ff]" },
    { name: "Organik / Pangan", value: data.composition.organic, color: "#66ccff", textClass: "text-[#66ccff]" },
    { name: "Residu TPA", value: data.composition.residual, color: "#cc0000", textClass: "text-[#cc0000]" }
  ] : [];
    ...
    <div className="min-h-screen bg-[#f8f9fb] font-sans pb-12">
      ...
      <div className="bg-white border-b border-slate-200 px-6 md:px-12 py-4 flex flex-col md:flex-row justify-between items-center gap-4 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold text-slate-800">Smart Waste Management Dashboard</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center border border-slate-200 rounded-lg px-3 py-2 bg-white">
            ...
            <div className="flex flex-col mr-6">
              <span className="text-[10px] text-slate-500 font-semibold leading-none mb-1">PERIODE</span>
              <select 
                value={period} 
          ...
          <div className="flex items-center border border-slate-200 rounded-lg px-3 py-2 bg-white">
            ...
            <div className="flex flex-col mr-6">
              <span className="text-[10px] text-slate-500 font-semibold leading-none mb-1">LOKASI</span>
              <select 
                value={campus} 
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-12 mt-8">
        ...
        <div className="bg-white rounded-2xl p-6 md:p-8 mb-6 border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-emerald-700 font-bold text-sm tracking-wide uppercase">{campus}</span>
            </div>
            <h2 className="text-2xl font-bold text-[#1a1f36]">Ringkasan Analitik Pengelolaan Sampah</h2>
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-slate-500">Gagal memuat data.</div>
        ) : (
          <>
            {/* SUMMARY CARDS */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm col-span-1 flex flex-col">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-lg font-bold text-[#1a1f36]">Komposisi Aliran Sampah</h3>
                    <p className="text-sm text-slate-500 mt-1">Distribusi pemilahan material daur ulang vs residu</p>
                  </div>
                ...
                <div className="relative h-[250px] w-full flex-grow">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span className="text-[10px] font-bold text-slate-400 tracking-wider">TOTAL TIMBULAN</span>
                    <span className="text-xl font-bold text-[#1a1f36]">{data.composition.total.toFixed(1)} kg</span>
                  </div>

                  {/* Labels on pie - very simplified for now since Recharts doesn't natively do exactly like SS easily without custom labels */}
                  {/* We rely on the legend below */}
                </div>

                <div className="grid grid-cols-2 gap-3 mt-6 bg-slate-50 p-4 rounded-xl">
                  {compositionData.map((item, i) => {
                     const pct = data.composition.total > 0 ? (item.value / data.composition.total) * 100 : 0;
                     return (
                      <div key={i} className="flex items-start gap-2">
                        <div className={`w-3 h-3 rounded-full mt-1 shrink-0`} style={{ backgroundColor: item.color }}></div>
                        <div>
                          <div className={`text-xs font-bold ${item.textClass}`}>
                            {item.name} ({pct.toFixed(0)}%)
                          </div>
                          <div className="text-[11px] text-slate-500 mt-0.5">{item.value.toFixed(2)} kg</div>
                        </div>
                      </div>
                     )
                  })}
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm col-span-1 lg:col-span-2 flex flex-col">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-lg font-bold text-[#1a1f36]">Peringkat Lokasi Timbulan Sampah Tertinggi</h3>
                    <p className="text-sm text-slate-500 mt-1">Diurutkan berdasarkan akumulasi volume sampah terkumpul</p>
                  </div>
                  <div className="flex items-center gap-1 text-xs text-slate-500">
                    <ArrowUpDown className="w-3 h-3" /> Urutan: Tertinggi ke Terendah
                  </div>
                </div>
                    ...
                    <div key={i} className="bg-slate-50/50 rounded-xl p-3 border border-slate-100">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <div className="text-xs font-medium text-slate-500">
                          Dominan: {loc.dominantName} <span className="font-bold text-slate-800 ml-1 text-sm">{loc.total.toFixed(1)} kg</span>
                        </div>
                      </div>
                      ...
                      <div className="text-[10px] text-slate-400 font-medium">
                        {loc.percentage.toFixed(1)}% dari total sampah di kampus 3
                      </div>
                    </div>
                  ))}
                  
                  {data.topLocations.length === 0 && (
                    <div className="text-center text-slate-400 py-10">Tidak ada data lokasi untuk periode ini.</div>
                  )}
                </div>
              </div>
            </div>
